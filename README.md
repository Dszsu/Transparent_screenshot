...
| **去除阴影遮罩**         | 移除窗口的阴影效果                                                         |
| **绕过焦点检查**         | 移除窗口焦点，绕过焦点检测（*注意：会导致无法唤出输入法*）                                    |
| **隐藏最近任务界面**       | 从最近任务列表中移除应用卡片                                                    |
| **修改窗口标题为**        | 将窗口标题伪装为系统录屏组件，支持全局品牌选择（OPPO/一加/真我、小米/红米、三星、华为EMUI、Vivo、魅族）或自定义标题 |
| **WebView 渲染窗口支持** | 自动检测系统 WebView 并跟随全局标题设置                                          |
| **WebView 壁纸**     | 使用 WebView 渲染的应用允许截屏至壁纸层（窗口透明时显示壁纸而非黑色）                           |
...
- 如果应用使用 WebView 渲染，窗口标题可能需要 WebView 子进程同时生效。模块会自动检测系统 WebView 包名。
- 如有需要可在设置中开启 **“使用 WebView 渲染的应用允许截屏至壁纸层”**。
- 如果对 WebView 单独设置了标题，则不再跟随全局设置。
